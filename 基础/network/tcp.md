...
三次握手是指在进行TCP连接时需要客户端和服务端总共发送三个包，目的是连接服务器指定端口，建立TCP连接并同步双方的序列号和确认号交换TCP窗口大小信息。

- 第一次握手（SYN=1，seq=x)
    客户端发送一个TCP的SYN标志位为1的包，指明客户端打算连接的服务器的端口，以及初始序列号x存放在序列号字段中
    发送完毕后，客户端进行 `SYN_SEND` 状态
- 第二次握手（ACK=1，seq=y,ACKnum=x+1）
    服务器发回确认包（ACK为1），服务端将自己的序列号放在seq中，同时将确认序列号设置为客户端序列号+1。
    发送完毕后进入 `SYN_REVD` 状态。
